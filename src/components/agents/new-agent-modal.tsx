...
export default function NewAgentModal({
  ...
  return (
    <Modal isOpen={open} onClose={onClose} className="max-w-lg p-8">
      <h3 className="mb-6 text-xl font-semibold text-foreground dark:text-white">Create new agent</h3>

      <div className="space-y-4">
        <div>
          <label className="mb-1 block text-sm font-medium text-foreground dark:text-white">Agent Name</label>
          <input
            value={name}
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-foreground dark:text-white">Agent Description</label>
          <textarea
            value={desc}
            onChange={(e) => setDesc(e.target.value)}
            rows={3}
            placeholder="Short description for your agent"
